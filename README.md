...
**v1.12.0「页面布局与可靠成品」**：独立页面布局卡片、准确的封面偏移、内页逐页位置、连续空格与超级紧凑；修复长封面压缩裁切、后退缓存与旧版本覆盖、资源事务误报及围栏标题解析。导出先生成真实 PNG 预览，下载复用同一文件；字体/几何/可观测字形墨迹独立校验。详见 [使用说明](USAGE.md) 与 [发布说明](docs/RELEASE-v1.12.0.md)。
