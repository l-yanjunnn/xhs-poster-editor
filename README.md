...
🌐 **当前线上版 v1.10.2；v1.11.0 发布准备中**：

- Cloudflare：https://xhs-poster-editor.l-yanjunnn.workers.dev
- 大陆通道：https://xhsposter.tshzchen.cn

🧪 **v1.11.0「封面副标题微排版」已完成本地实现并锁定正式视觉值，待补双轨生产证据**：右侧「封面版式」新增副标题字距「紧凑 / 标准 / 舒展」三档，只作用于首图封面副标题。普通与居中版式的紧凑 / 舒展为 `0em / 0.08em`，小字在上大字在下（kicker）为 `0.10em / 0.26em`；标准档不新增覆盖，完整继承 v1.10.2 视觉。右栏「H1 宽度」同时澄清为「全篇 H1 宽度」，行为与左侧 H1 编辑入口均未改变。发布准备与待补证据见 [`docs/RELEASE-v1.11.0.md`](docs/RELEASE-v1.11.0.md)。
