...
🧪 **本地 v1.7.1 候选版（未发布）**：预览与 PNG 导出改为共享同一份确定性行级快照；中文标点按相邻字形的实际可见墨迹求解左右净空，双引号、冒号、顿号和连续标点不再依赖固定半字宽或单侧压缩，行末按可见右缘透明悬挂；汉字与数字/拉丁边界、统一汉字残差、混排基线和逐行装饰都由可测量的布局层生成。字体、基线或几何预检失败时会阻止导出并明确提示。验收材料见 [`docs/v1.7.1/V1.7.1-VALIDATION.md`](docs/v1.7.1/V1.7.1-VALIDATION.md)。当前线上仍为 v1.7.0。
