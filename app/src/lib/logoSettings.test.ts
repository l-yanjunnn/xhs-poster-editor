...
describe('Logo scope and stable page ownership', () => {
  ...
  it.each(['inner', 'outside-area'] as const)('retains explicit pages and restores %s through history and JSON', position => {
    const e = create()
    ...
    e.view.dispatch(closeHistory(e.state.tr.setDocAttribute('logoSettings', { ...settings, strategy: 'none', coverPosition: position })))
    expect(read(e)).toEqual(['inherit', 'inherit', 'show'])
    const restored = new Editor({ extensions: createEditorExtensions(), content: e.getJSON() })
    expect(restored.state.doc.attrs.logoSettings).toMatchObject({ strategy: 'none', coverPosition: position })
    ...
    e.commands.redo(); expect(e.state.doc.attrs.logoSettings.coverPosition).toBe(position)
  ...
  it('keeps legacy defaults and only saves visual cover position in themes', () => {
    ...
    expect(normalizeTheme({ ...BUILTIN_THEMES[0], coverLogoPosition: 'outside-area' })?.coverLogoPosition).toBe('outside-area')
