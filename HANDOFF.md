...
> 最后更新：2026-08-12 晚（**v1.8.0「长文双向滚动联动版」发布闭环**；同日完成 v1.7.3 发布闭环与 **Gate 0 CDN 字体诊断（疑点不成立，见 §5 与 `docs/FONT-GATE0-2026-08-12.md`）**。旧版全文在 git 历史，`git log -- HANDOFF.md` / `git show <commit>:HANDOFF.md` 可考古）
...
| 项 | 值 |
|---|---|
| 线上版本 | **v1.8.0**。发布提交/tag 为 `067a761` / `v1.8.0`；Cloudflare 与 OSS/CDN 均加载 `assets/index-37OJxpyY.js` + `assets/index-DAFB0ykm.css`。JS SHA-256 `dddc196e8b68f1994376f1e7a0a50fd2053875e7822e489fde09ac030084f5a7`，CSS SHA-256 `3d6e3e31ac3c47486e33e3c73d37bf30de59f759997680c68e657d29a2714723`，两入口与 `archive/dist-v1.8.0/` 一致 |
...
### 迭代待办快照（2026-08-12 晚梳理；完整规格与实施边界以 `docs/ROADMAP-2026-08-12.md` 为准）

> v1.8.0 上线后 P0/P1 已清零。下表是全部未做事项；新会话从这里挑活，动手前读 ROADMAP 对应行。

| 优先级 | 待办 | 一句话范围 |
|---|---|---|
| **P2·小改动，下个版本顺手带** | **荧光笔默认透明度 50% → 25%**（2026-08-12 用户提出） | 改 `app/src/lib/textHighlight.ts:2` 的 `TEXT_HIGHLIGHT_DEFAULT_OPACITY`，同步 §8 冻结文案、USAGE.md 与断言默认值的测试；不迁移已有草稿的既选透明度 |
| P2 | 结构化封面槽位与三套版式 | 主/副标题槽位；左对齐叠排、居中海报、小字在上大字在下；只开放有限位置调整，不做自由文本框与复杂网格 |
