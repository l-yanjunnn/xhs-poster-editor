...
桌面 UI/UX 提案 v1：[`docs/design/v1.4.0/xhs-editor-v1.4-ui-baseline-v1.png`](docs/design/v1.4.0/xhs-editor-v1.4-ui-baseline-v1.png)。这是下一版的视觉与交互基线，不是已经完成的产品截图；用户目检后仍可调整。
...
- 当前视觉提案选择「中央 9:15 成品画布直接选图、缩放与吸附」，左侧负责正文录入和结构编辑，右侧负责所选对象参数；这是推荐方向
- 若用户确认这张视觉提案，稳定 `imageId`、复制粘贴去重、`Preview imageId → Tiptap node position` 映射及预览比例坐标换算自动升级为 v1.4 必做；中央画布是唯一图片直接操作面，左侧不得再维护第二套独立选框/拖动状态
- 若用户否决中央画布直改，则退回「左侧 Tiptap NodeView 直接操作、中央画布只预览」；两条路线只能选一条。无论选哪条，Tiptap 都是唯一数据源，Preview DOM 不得成为持久数据源
...
1. **v1.4.0 桌面交互与可靠性版（下一版）**：图片直接操作、等比缩放、左中右对齐、磁吸、排版参考线、桌面编辑器外壳、轻量最近操作、草稿与导出可靠性，以及上述沃林 UX 原则
