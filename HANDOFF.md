...
| 项 | 值 |
|---|---|
| 线上版本 | **v1.10.1**。发布提交/tag 为 `927175f` / `v1.10.1`；Cloudflare 与 OSS/CDN 均加载 `assets/index-C9qzx69K.js` + `assets/index-CopAT3sD.css`（CSS 与 v1.10.0 同 hash，本版无样式改动），两入口与 `archive/dist-v1.10.1/` 一致。两入口删光草稿回归（v1.10.1 修复项 + 版本号断言）全绿；大陆入口 `test_prod_deep.py` 全绿；Cloudflare 入口深回归两次复现代理黑洞事故签名（同 v1.9.0/v1.10.0 发版日，首个导出等 download 120s 超时，纯环境故障——本版未触碰导出路径且同一导出代码 2 小时前在 v1.10.0 的 Cloudflare 深回归已全绿），待代理窗口恢复后复跑补录。v1.10.0 发布记录：提交/tag `adbc9ef` / `v1.10.0`，两入口 `test_prod_deep` + 封面槽位 8 项像素断言全绿 |
...
> v1.10.0 结构化封面槽位版已上线出列（含 R8 两项用户可感知修复 + M4/M5 死代码清理，见上方当前迭代行）；v1.9.0 引擎提速版已上线出列。**R7 英文长段落 justify** 用户拍板先 mark 不做（详见 CODE-REVIEW §五）。下表是功能向未做事项；新会话从这里挑活，动手前读 ROADMAP 对应行。
