...
> v1.8.2 性能版已上线出列（P2/P3/P4/P6/M1/M2 全部完成，详见 CODE-REVIEW §五对应行的「已实现待发版→已发版」标注）。**下一个版本建议**：功能版首选「结构化封面槽位」（下表 P2 首行），或按 CODE-REVIEW 排期做 v1.9.0（字体本地化 + P1 DP 求解器 + P5/P7 基线批量化，配 profile）。下表是功能向未做事项；新会话从这里挑活，动手前读 ROADMAP 对应行。
