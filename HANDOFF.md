...
### v1.4.0「图片交互可靠性版」（2026-08-10 用户拍板；新窗口执行）
...
1. **图片选中与直接缩放**：点击正文图片出现清晰选中框与缩放手柄；拖手柄等比缩放，宽度限制 10%–100%；一次拖动只形成一次撤销记录和一次草稿更新
...
5. **编辑层不进成品**：Tiptap NodeView 的选中框和缩放手柄只存在于编辑器外壳，不得序列化；进入右侧 `.page` 的版心参考线、磁吸线或未来右侧选框统一使用真实 DOM + `data-preview-only`，导出 PNG 必须主动剥离，不能使用难以排除的伪元素
...
- 推荐抽出独立 Image Extension + React NodeView：Pointer Events 拖动时只改临时样式，`pointerup` 时一次性 `updateAttributes`；`pointercancel / Esc / 失焦` 要能回滚。NodeView 外壳不得改变正式 `renderHTML`，序列化仍是单个根级 `<img>`
- 不要直接启用 Tiptap 自带像素 resize：编辑区、1080 画布、40% 预览是三套尺度，必须继续存百分比
- 若用户拍板要在**右侧成品预览上直接点图拖动**，仍以 Tiptap 为唯一数据源，但要先补稳定 `imageId`、复制粘贴去重、`Preview imageId → Tiptap node position` 映射桥及 40% 坐标换算；不得把 Preview DOM 当数据源或持久写入，临时覆盖层由 React 状态驱动、最终只向 Tiptap 提交，也不得用图片数组下标/页码当 id

**操作界面重设计基线：线框图是 v1.4.0 必交付，完整桌面/手机壳的编码范围待用户目检拍板：**

- 桌面端学习沃林的层级：**全局动作在顶部、正文编辑在左、成品画布居中、当前对象属性在右**。顶部只留版本/撤销重做/草稿状态/参考线/导出等全局动作，不再横向塞满所有字体和图片参数
- 右侧改为上下文检查器：未选对象时显示页面/主题；选中文字时显示排版；选中图片时只显示对齐、宽度、替换、删除。高级项按需展开，不让非设计用户一次看到全部控制
- 画布上的悬停轮廓、选中框、手柄、临时蓝线与状态文案必须明确区分「可点 / 已选 / 正在吸附」；禁用项要直接说明原因
- 手机端不要继续把桌面顶栏压成多行；学习沃林的**同一引擎、设备专属外壳**：顶部保留撤销/重做/导出，画布优先，底部用「内容 / 样式 / 素材 / 页面 / 更多」页签和可收起抽屉
- 先做桌面与手机关键路径线框和截图对照，再决定 v1.4.0 一次落全壳，还是把手机壳拆到后续大版本；无论如何都不改现有排版与导出引擎

**v1.4.0 必测：**旧草稿兼容；非法宽度/对齐归一化；`getJSON → setContent → getHTML` 后仍是单个 `<img>` 且分页不变；`resolveContentImages` 后 `width / align / assetId` 不丢；左中右与缩放在编辑器/预览/导出三处一致；缩放不触发节点重排；一次手势一次 undo 且 redo 可恢复；磁吸阈值与 Alt 旁路；Esc/失焦/pointer cancel 回滚；多图只改选中项；第二页图片不改变分页；刷新/另存恢复；所有辅助层和溢出提示不进导出；手机手柄命中范围。若启用右侧 Preview 桥，再测 `imageId` 复制粘贴去重、第二页映射与 40% 坐标换算。

### 从沃林继续学习、但不与 v1.4.0 强行捆绑

1. **有名称的修改时间线**：「挪动 / 改大小 / 换主题」按步骤回跳；它与真正草稿是两层概念，不能用「最近导出」替代 v1.3 已有草稿
2. **面向普通用户的容错**：素材/字体缺哪个只降级哪个，给可执行的重试提示；必要时再建设独立自诊断页
3. **主次操作层级**：导出保留强文字 CTA；次级按钮可图标化但必须有中文提示；快捷键只做增强，不作为发现功能的主路径
4. **意图出现后再加载重功能**：用户打开对应面板时再预热，避免首屏为暂时不用的能力付出性能成本
5. **受控自由度**：优先分段档位、吸附和安全边界，不把 Figma/Adobe 的全部复杂度搬给非设计用户

### 其余后续候选（未拍板，不混入 v1.4.0）
...
3. **PWA**（vite-plugin-pwa，可安装到 Dock）
4. **字体冗余清理**：删 fontsource 的 .woff 只留 .woff2，dist 体积约减半（115MB → ~60MB）
5. **Tauri 打包** macOS .app
6. **Markdown 导入 → 自动编排 → 自动分页**：作为同一轮后续能力设计；当前继续手动分页
7. **hasRaceArtifact/retry 简化**（v8 稳定运行数月后可清）
8. **封面专属标题 + 钩子正文布局**：先积累模板，不混进图片交互版本
9. **公考专属模板**：待编辑器交互稳定后再设计，不与基础能力迭代混版
