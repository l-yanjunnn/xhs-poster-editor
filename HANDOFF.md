...
> 最后更新：2026-08-13 凌晨（**v1.8.1「审查修复版」发布闭环**；同日完成全量代码审查，报告见 `docs/CODE-REVIEW-2026-08-13.md`。旧版全文在 git 历史，`git log -- HANDOFF.md` / `git show <commit>:HANDOFF.md` 可考古）
...
| 项 | 值 |
|---|---|
| 线上版本 | **v1.8.1**。发布提交/tag 为 `dffd5cd` / `v1.8.1`；Cloudflare 与 OSS/CDN 均加载 `assets/index-BPzi9LFD.js` + `assets/index-DAFB0ykm.css`，两入口与 `archive/dist-v1.8.1/` 一致。两入口 `test_prod_deep.py`（三主题导出像素 + 用户字体）全绿 |
...
> v1.8.1 上线后：荧光笔 25% 已完成出列。**下一个版本建议是 v1.8.2 性能版**（渲染性能三连 + WAL 防抖 + collectAllCss 缓存 + 删 exportPages 死管线 + 预检门控去分叉，全部条目及 file:line 见 CODE-REVIEW §二/§三）。下表是功能向未做事项；新会话从这里挑活，动手前读 ROADMAP 对应行。
