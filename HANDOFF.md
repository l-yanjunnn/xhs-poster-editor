# v1.14.0 上线闭环已完成（2026-09-17 21:21 北京时间）

用户已审阅 rc.2 并授权发布。本版整个 Logo 优化已双轨上线；下方候选和旧版本记录均为历史。

- **发布提交 / 精确 tag**：`e7d4e68d95333da79935162499712c0fb789c011` / `v1.14.0`，已推送；归档与文档提交不移动此 tag。
- **正式入口**：https://xhsposter.tshzchen.cn ／ https://xhs-poster-editor.l-yanjunnn.workers.dev 。两端 HTML / JS / CSS 与正式构建逐字节一致，bundle `index-Dq_yHRf4.js`、`index-K8e2YxU7.css`。
- **本版变化**：统一 Logo 卡片；整篇四范围 + 本页跟随／显示／隐藏，单页覆盖优先、覆盖计数与可撤销恢复跟随；封面三档位置，范围外档在中心裁切上沿之外留 5 画布像素。旧稿默认、正文布局和特殊主题约束保留，手动选页逻辑不变。
- **验证**：49 文件 / 517 单测、类型/build/lint 通过。两个正式入口各完成三档9张PNG，全部与对应成品预览逐字节一致；历史、刷新、隐藏显示、模板/缩略图、键盘、窄屏通过。三主题、自定义字体、18/19页导入与19张2160×3600完整ZIP通过；Logo移动前后正文区域像素完全相同。
- **测试清理限制**：OSS长文与Cloudflare主题回归在所有功能断言完成后卡在 Chrome 153 的 context 关闭；保留首轮/复跑日志和下载，独立复验ZIP/PNG后停止测试进程。记录为功能通过、清理异常，不虚记进程正常退出；应用代码未因此改动。
- **使用说明**：权威飞书文档已增加17段说明、2张正式入口合成稿截图与图注，并补充原滚动联动章节；全部回读确认。讲清三档区别、5px含义、覆盖优先级/恢复、手动选页、模板、分页和模拟裁切边界。USAGE.md、README和RELEASE同步。
- **公告**：核验 default / `cli_a92bb2ebb1795bd2`「Claude聊天助手」及刘彦君既有1v1后，发送完整独立公告；消息 `om_x100b658adfe78510c101b657ec257bf` 已全文回读一致，含双入口与权威使用说明链接，无群发。
- **归档与证据**：`archive/dist-v1.14.0/` 不覆盖旧版，核心哈希与生产一致。摘要 `docs/PRODUCTION-v1.14.0.json`；脚本、日志、截图、真实下载、飞书回执均在 `/Users/a0000/Codex_YJ/tasks/xhs-logo-20260917/production-v1.14.0/`。
- **边界**：桌面Chrome隔离合成稿；中心3:4只是编辑器模拟，不代表真实平台所有设备/发布方式。原作者稿、其他浏览器与原生目录选择器未新增验收。不操作真实用户草稿。

---

# 历史候选：v1.14.0-rc.2 本地候选（2026-09-17 北京时间，未发布）

新增第三档「封面可见范围外右上角」：Logo 底边在中心裁切参考上方 5 画布像素，完整图保留。旧两档和手动选页逻辑不变。517 单测、lint、类型/build 和真实浏览器9张PNG下载验证通过。新设置的历史、刷新、模板缩略图/应用、隐藏显示、键盘与窄屏验证通过。

详见 `docs/LOCAL-CANDIDATE-v1.14.0-rc.2.md`；任务目录 `REPORT-rc.2.md`、`BUILD-MANIFEST-rc.2.json` 与 `evidence/rc2/`。下方 rc.1 是前一候选记录，不是当前版本。无发布授权，等待本地审阅。

---

# 2026-09-17：v1.14.0-rc.1 Logo 优化本地候选（未上线）

本节仅描述隔离工作树候选。正式版仍为下方 v1.13.0；本轮没有合并、推送、部署或对外发送。

- 工作树：`/Users/a0000/Codex_YJ/tasks/xhs-logo-20260917/candidate`；分支 `codex/xhs-logo-20260917`；基线 `1cd7d33`（正式发布 tag `v1.13.0` 后的归档提交）。原仓库既有改动和历史版本保留。
- 交付：封面 Logo 两位置；整篇四规则 + 逐页覆盖；统一 Logo 卡片；覆盖计数和可撤销的恢复跟随；草稿/模板/历史/导出一致；公考主题约束保留。
- 可审预览：`http://127.0.0.1:4198/`，启动 `../START-LOCAL.command`。项目使用系统 UI 字体。依赖链接复用原仓库已锁定 node_modules，不修改共享依赖。
- 必读：`docs/LOCAL-CANDIDATE-v1.14.0-rc.1.md`（数据与分页规则），任务上级 `REPORT.md`（证据与限制）、`BUILD-MANIFEST.json`（候选身份和构建哈希）。可重跑浏览器脚本在 `tools/logo-candidate/`。
- 验证：49文件516单测、类型/build/lint通过；真实Chrome合成稿流程、三主题两位置18张PNG以及构建版6张PNG，预览与下载逐字节一致；封面覆盖更新使旧成品失效而文字快照不变；旧版显/隐顶部稿几何兼容。
- 窄屏：沿用原产品1280px最小工作区；900/600/390px下无新增横向溢出，Logo卡片内部不溢出。不是完整手机编辑器。
- 等待用户实际审阅；后续发布需另行授权并完成原项目双入口生产验收。中心3:4参考不代表小红书所有场景真实裁切。本地合成稿不替代真实作者原稿或生产验收。
...
- 追加需求：逐页内页滑杆（±360 px，步长 1 px），保留默认 / 顶部 / 居中，点击固定选项清零；只移动内容，Logo 固定。逐页偏移保存在分页节点，模板默认偏移与草稿同步；旧数据缺省 0。底部参考线改用画布高度减主题底部留白，脱离内容容器偏移。
- 用户明确要求：主题留白/安全区仍为普通 warning；`content-clipped` 保留强制导出按钮，独立浮窗显示受影响页/段、边界放大图与实际裁切 PNG，确认后下载。其他字体、完整性、压缩、重叠阻断不放宽。
- 实现与验收完成：构建、lint、505 单测；内页正负微调、预设重置、拖动撤销重做、草稿刷新与实际 PNG/ZIP 一致通过；模板继承/显式 0 与三主题固定底部参考线通过；Chrome 153 实际构建裁切确认→ZIP，浮窗 PNG 与下载逐字节一致；取消进行中的预览、多裁切页、移动窄屏、修改原稿失效；真实 OPFS 目录权限边界、写入中断续传；三主题与自定义字体；完整19张2160×3600 PNG ZIP。
- 证据与可重跑脚本：任务上级目录 `checks/`、`evidence/`，见 `../REPORT.md`。本地构建预览 http://127.0.0.1:4188/ ，开发验收 http://127.0.0.1:4187/ 。开发端依赖通过符号链接复用同机已有锁定依赖，不更改共享依赖。
- 未推送、未部署、未发送公告。本轮未得到上线授权；若后续发布，必须走双入口真实生产验收，当前本地通过不能替代生产验证。原周宇九页原稿未取得，不把合成裁切测试说成原稿已修复。
...
## 2026-09-14 内页位置与模板默认（本地验收历史，用户 review 已通过）

用户已确认三个选项的语义：**默认跟随模板保存值；顶部与右上角 Logo 上缘对齐；居中保留原有留白，不作几何居中；去掉底部**。本轮已实施在同一隔离候选分支，仍未推送、部署或发送公告，等待用户实际 review。

- 新增模板/草稿样式字段 `innerVerticalDefault`；旧数据缺省 middle 保持原默认外观。在内页保存模板时取该页有效位置，选封面时沿用模板默认；面板明示保存值。保存事务成功后才激活，已离开的草稿/主题不会被晚到的保存改写。
- 逐页 HR 身份与覆盖仍在原模型内：显式 top/middle 不受切换模板影响，inherit 跟随新模板；撤销/重做与刷新恢复通过。早期候选 bottom 归一化为 inherit；早期候选的 middle 按新语义解释。
- 顶部沿用原内容流，仅按 Logo 锚点移动。发现长首段遮住 Logo 的实际失败后，为首段预留 Logo 宽度/高度；其后块保留完整宽度。独立几何检查增加 Logo 与文字/图片重叠阻断。隐藏 Logo 保留对齐锚点。
- [本轮报告](/Users/a0000/Codex_YJ/tasks/xhs-inner-template-layout-20260914/REPORT.md) 与该目录 checks/、evidence/ 为本轮证据。当前运行构建在 app/dist，地址 http://localhost:4185/ 。源码/构建哈希见本轮 BUILD-MANIFEST.json。
- `archive/dist-v1.12.0/` 是 4d04039 中的**旧发布准备快照**，未上线且不含本轮语义修正；保留不覆盖，不能拿它发布。待最终 review 通过后另建清楚命名、不可覆盖的最终构建归档。
- 发布准备不等于用户 review 通过。之前 GitHub device code 已过期；需发布时重新核实认证。Z5/Z6 原稿、异常原图及原作者环境缺口仍在，不因本轮合成稿成功而关闭。

---

## 2026-09-14 试用反馈优化与 v1.12.0 发布准备（前轮，以下构建已被新候选取代）

用户已明确要求完成页面布局优化、review、导出检查后走上线闭环，覆盖上一轮「仅本地候选」限制。本轮已完成本地 review 与回归，**尚未部署**：GitHub CLI 旧认证失效，已开启官方 device flow 等待用户完成登录；OSS 目标可读，未提前单轨发布。

- 右侧页面布局统一使用 InspectorCard；先选页并定位预览，封面滑杆仅在封面显示，内页提供独立四档位置，整篇空格单独分区。
- 新发现并修复候选版偏移根因：只增减顶部 padding 导致居中位移减半、底部不动；现在在既有 relative 内容盒上整体位移，保持排版空间不变。三版式 × 三对齐均验证正负 80px 精确移动，内页不变。
- 正式构建 v1.12.0，bundle `index-DWFdcnrj.js`；类型/构建/lint、488 单测、9 组偏移真实 PNG、54 组长封面、三主题/自定义字体、19 页 ZIP、独立漂移/丢字注入通过。成品预览和下载 PNG 字节一致。
- 本轮 [Review](/Users/a0000/Codex_YJ/tasks/xhs-editor-release-20260914/REVIEW.md)、evidence/ 与 checks/ 位于同一任务目录。源码基于 91d277c；原仓库 HEAD/已有未跟踪文件保留。`archive/dist-v1.12.0/` 为本地准备归档，尚无发布 tag。
- 下一步只缺 GitHub 认证后继续推送、双轨部署和两个真实生产入口回归，之后固定 tag / 更新状态 / 完成固定机器人向用户既有 1v1 公告。公告稿在 docs/RELEASE-v1.12.0.md，发前必须更新真实发布状态。
- Z5/Z6 原始九页稿和原作者浏览器仍缺；原生系统目录 UI 及其他平台未完成新验证。不要将本地或模拟通过写为原稿修复或生产已验收。
...
| 项 | 值 |
|---|---|
| 线上版本 | **v1.14.0**，发布提交/tag `e7d4e68d95333da79935162499712c0fb789c011` / `v1.14.0`。双轨、生产验收和公告完成，见本文顶部与 docs/PRODUCTION-v1.14.0.json。 |
| 本地归档 | `archive/dist-v1.14.0/`；全部旧版本归档保留不覆盖。完整复原走 v1.14.0 tag + bash ci.sh。 |
| 状态 | **v1.14.0 双轨上线闭环完成**。Logo 三档封面位置、整篇与逐页显隐、统一设置区；手动选页行为保留，权威使用说明已含详细操作与截图。测试清理限制详见顶部记录。 |
...
| 最近完成迭代 | **v1.11.0「封面副标题微排版」2026-08-19 已双轨上线，2026-08-20 已完成公告闭环**。产品/视觉/发布提交为 `919802a` / `2052473` / `5c6ccf9`，精确 tag 为 `v1.11.0`。双入口与本地逐字节同构，两组 `test_prod_deep` 和两组公考 3×3 真导出矩阵均全绿；`archive/dist-v1.11.0/` 已归档。12:33（北京时间）已使用 `default` 租户「Claude聊天助手」向刘彦君既有 1v1 私聊独立发送完整公告，正文含双工具地址与「小红书排版编辑器 · 使用说明」链接；消息 `om_x100b675c77e87ca8debe2118f437412` 已回读确认，是本版唯一可转发的正式公告。 |
...
> **状态边界**：2026-08-19 用户确认完整范围，随后以「我们走上线闭环」锁定当前视觉值并授权完整发布闭环。v1.11.0 已完成 WP1–WP4、本地门禁、精确 tag、双轨部署、双入口生产回归、不覆盖归档与机器人私聊公告；本节保留原实施规格和最终证据。2026-08-20 用户再次澄清：本项目语境中的「走上线闭环」本来就包含使用固定机器人向其既有 1v1 私聊发送专业公告，无需重复等待“代发”二次口令。
...
- 授权边界：规划阶段不得 bump、tag、归档、部署或发公告；用户明确说「走上线闭环」「可以发布」或同义表达后，才执行发布。对本项目而言，该授权包含使用 `default` 租户「Claude聊天助手」向刘彦君既有 1v1 私聊发送专业公告；公告必须显式带权威使用说明链接 `https://icnyqonxxzop.feishu.cn/docx/SFDddCFb3o8T5VxP2wBcb0XXndd`。发送前仍须核验 profile/app/chat 和文档标题/token，发送后回读全文和真实消息 ID，不发群聊或其他联系人。若用户当次明确说只备稿或不发送，则以当次指令为准。
...
4. 只有用户另行明确授权，才 bump 版本、提交、tag、双轨部署、双生产入口回归、不覆盖归档并完成机器人私聊公告；本版在「我们走上线闭环」后执行。公告使用固定机器人和既有 1v1 会话，正文必须带权威使用说明链接；写操作前核验身份、目标和文档链接，写后回读，不扩散到群聊或其他联系人。
...
- [x] `archive/dist-v1.11.0/` 不覆盖归档与 README/USAGE/HANDOFF/RELEASE 文档闭环（2026-08-19）
- [x] `default` 租户「Claude聊天助手」向刘彦君既有 1v1 私聊独立发送含双工具地址与权威使用说明链接的完整公告并回读（2026-08-20 12:33；正式消息 `om_x100b675c77e87ca8debe2118f437412`）
...
- 归档与公告：`archive/dist-v1.11.0/` 已以不覆盖方式生成（9.6M、15 个归档文件＝14 个应用文件 + 1 份字体 manifest，排除 3337 个可由 tag + 安装恢复的字体文件），核心资产哈希与生产一致。2026-08-20 12:33（北京时间）已核对 `default` profile、app `cli_a92bb2ebb1795bd2`「Claude聊天助手」、刘彦君既有 1v1 私聊 `oc_9e2813855736a171c86c8df2b13e1048`，以及权威文档标题/token；随后作为独立消息发送 `docs/RELEASE-v1.11.0.md` 所载完整公告。正式消息 `om_x100b675c77e87ca8debe2118f437412` 已回读确认，`displayLabel=[Bot] Claude聊天助手`，正文含双工具地址与使用说明链接，是本版唯一可转发版本。12:19 的不完整公告 `om_x100b675c4a2154a0c326e72ea070b91` 和 12:26 的补充回复 `om_x100b675c505348a0c49f0c4a6532e82` 仅保留为纠错记录；用户自行转发正式消息。
- 用户文件：`docs/飞书使用说明-文档封面.png` 始终保持未跟踪，未纳入、移动、覆盖或删除；高分辨率本地视觉证据也保持可再生的本地证据，未整体塞入 Git 历史。

##### 六、新窗口接手提示

> v1.11.0「封面副标题微排版」已完整上线并完成机器人私聊公告，无待执行发版步骤。后续版本若用户明确说「走上线闭环」「可以发布」或同义表达，应把使用固定机器人向刘彦君既有 1v1 私聊发送专业公告视为闭环步骤，无需再追问“代发”；公告正文必须带权威使用说明链接 `https://icnyqonxxzop.feishu.cn/docx/SFDddCFb3o8T5VxP2wBcb0XXndd`。仍须写前核验身份、目标与文档标题/token，写后回读并记录真实 `om_` ID，且不发群聊或其他联系人。若用户当次明确说只备稿或不发送，则不发。后续产品工作从 P2/P3 候选重新确认范围，继续保留用户未跟踪文件 `docs/飞书使用说明-文档封面.png`，不纳入、移动、覆盖或删除。
...
8. **发版公告必须专业、完整**（含双工具网址 + 权威使用说明链接 + 本版变化摘要 + 必要注意事项）。当前使用说明为「小红书排版编辑器 · 使用说明」：`https://icnyqonxxzop.feishu.cn/docx/SFDddCFb3o8T5VxP2wBcb0XXndd`；发送前按标题和 docx token 复核，不能只写“使用说明已更新”却漏掉地址。用户明确授权「走上线闭环」「可以发布」或同义表达时，公告属于闭环固定步骤：使用飞书旧企业 `default` 租户的「Claude聊天助手」发送到刘彦君既有 1v1 私聊，无需再索取“代发”二次口令；发送前核对 profile、app_id 与 chat，发送后回读全文并只记录真实 API 回执 ID。不得发群聊或其他联系人，用户自行转发；若用户当次明确要求只备稿或不发送，则不调用机器人
...
- 发版公告要写得专业、可直接发送，且必须显式包含双工具网址和权威使用说明链接 `https://icnyqonxxzop.feishu.cn/docx/SFDddCFb3o8T5VxP2wBcb0XXndd`；用户明确授权「走上线闭环」「可以发布」或同义表达时，固定机器人 1v1 公告属于闭环步骤，无需重复询问“代发”。发送前核对 `default` profile、「Claude聊天助手」、刘彦君既有私聊及文档标题/token，发送后回读真实消息 ID；不发群聊或其他联系人，用户自行转发；当次明确只备稿或不发送时不发
- 终端统一 iTerm2
