# 2026-09-14 23:52 北京时间：v1.12.1-rc.1 裁切二次确认（本地候选，未上线）
...
- 实现与验收完成：构建、lint、501 单测；Chrome 153 实际构建裁切确认→ZIP，浮窗 PNG 与下载逐字节一致；取消进行中的预览、多裁切页、移动窄屏、修改原稿失效；真实 OPFS 目录权限边界、写入中断续传；三主题与自定义字体；完整19张2160×3600 PNG ZIP。
