...
> 最后更新：2026-08-13（**v1.8.2「性能版」发布闭环完成**；需求源为 `docs/CODE-REVIEW-2026-08-13.md` §五版本编排。旧版全文在 git 历史，`git log -- HANDOFF.md` / `git show <commit>:HANDOFF.md` 可考古）
...
> v1.8.2 性能版已上线出列（P2/P3/P4/P6/M1/M2 全部完成，详见 CODE-REVIEW §五对应行的「已实现待发版→已发版」标注）。**2026-08-13 用户拍板：v1.9.0 立项动工**——范围 = 字体本地化 + P1 DP 求解器 + P5/P7 基线批量化（配 profile），并入 M7 App.tsx 拆分、M3 hash 合并、R5 打开草稿冗余落盘；before 基线脚本在 `tools/perf/`（solver-bench.mjs + browser-baseline.py），数字落 `docs/PERF-BASELINE-2026-08-13.md`，优化后同脚本复跑对比。**结构化封面槽位**已出三套版式视觉 demo（Artifact，2026-08-13）待用户目检，过目认可后再立项动手。**R7 英文长段落 justify** 用户拍板先 mark 不做（详见 CODE-REVIEW §五）。下表是功能向未做事项；新会话从这里挑活，动手前读 ROADMAP 对应行。

| 优先级 | 待办 | 一句话范围 |
|---|---|---|
| **P2·下一个功能版首选** | 结构化封面槽位与三套版式 | 主/副标题槽位；左对齐叠排、居中海报、小字在上大字在下；只开放有限位置调整，不做自由文本框与复杂网格 |
| P2（≥v1.9.0） | 字体本地化、减重与字重档位 | Gate 0 已定性为纯性能优化（非缺陷，见 §5）；第一刀砍 LXGW 的 6 份冗余档位 CSS（~176 KB），按真实使用率拆批 |
