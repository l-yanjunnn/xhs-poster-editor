...
| 发布闭环 | **2026-08-11 全部完成**：15:48 CST 完成 `main` / 精确 tag 推送、Cloudflare + OSS/CDN 双轨上线、两入口 1/2/5 页公考矩阵及旧三主题/用户字体深回归；16:10 CST 使用飞书旧企业租户的既有机器人将 `docs/RELEASE-v1.5.0.md` 专业公告发送至刘彦君 1v1 私聊并回读确认；Markdown 导入/自动编排继续顺延 |
...
3. **v1.6.0 视觉资产质量版（候选，0%，先诊断再立项）**：标题字重档位、ZCOOL / Ma Shan Zheng / Long Cang 等字体本地化、字体冗余清理、字体与图片在预览/导出中的一致性、加载性能优化。Gate 0 先验证 §5 的五类 CDN 字体疑点；若只是单点线上缺陷，版本应评估为 `v1.5.1`，不要为了沿用路线图强行扩成 v1.6
...
§8 的 v1.5.0「公考双底图模板版」已于 2026-08-11 完成 Cloudflare + OSS/CDN 双轨上线，发布提交/tag 为 `227b0da` / `v1.5.0`，两轨均加载 `index-D0LqeUgP.js`。生产 1/2/5 页公考矩阵、旧三主题、用户字体与字形中线回归全绿，归档在 `archive/dist-v1.5.0/`；专业公告也已通过飞书旧企业租户机器人发到刘彦君私聊。v1.5 当前无未完成步骤。

下一版本尚未正式立项。请先执行 v1.6 Gate 0 范围诊断，不立即改码：

1. 在两个生产入口验证 ZCOOL、马善政、Long Cang、LXGW、Inter 的预览与 PNG 导出是否一致；
2. 盘点字体网络请求、构建体积、重复字重与加载性能；
3. 设计标题字重从 H1/H2/H3 二态升级为档位时的 Theme/草稿向后兼容方案；
4. 给出可独立验收的工作包、测试矩阵和版本判断，先与用户讨论确认。

若只确认一个现有字体导出缺陷，评估发 v1.5.1；若同时实施字体本地化、字重档位和性能优化，再立项 v1.6.0。Markdown/自动编排属于 v1.7，不混入本轮。
```
