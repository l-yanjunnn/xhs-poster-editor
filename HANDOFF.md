...
> 最后更新：2026-08-12（v1.7.1 确定性排版已发布；生产门禁发现列表序号导出基线回归后，已在本地修复并升版为 v1.7.2 候选。尚待重新部署、生产回归、归档与公告。旧版全文在 git 历史，`git log -- HANDOFF.md` / `git show <commit>:HANDOFF.md` 可考古）
...
| 本地候选 | **v1.7.2，待重新发布**。v1.7.1 的共享行级快照、标点双侧净空、连续标点、行末可见右缘、混排 baseline、逐行装饰、精确字体预检与导出硬门禁全部保留；v1.7.2 新增列表 marker 与首个确定性正文 atom 的同 renderer 基线位移，旧生产像素门禁已由 12px 恢复为 0px。验证报告：`docs/v1.7.1/V1.7.1-VALIDATION.md` |
...
| 当前迭代 | **v1.7.2 正在完成已授权上线闭环**。自动测试、类型检查、ESLint、构建、最小排版样例和真实 19 页完整导出均通过；生产门禁发现的列表序号基线回归已修复，本地生产构建的 1/2/5 页旧链重新全绿。原生长文、字体全集本地化/减重/字重档位/结构化封面、37+ 上传分组整体重均衡与跨刷新目录续写继续延期 |
