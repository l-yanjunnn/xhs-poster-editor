...
| 项 | 值 |
|---|---|
| 线上版本 | **v1.10.0**。发布提交/tag 为 `adbc9ef` / `v1.10.0`；Cloudflare 与 OSS/CDN 均加载 `assets/index-Dzu6JFfe.js` + `assets/index-CopAT3sD.css`，两入口与 `archive/dist-v1.10.0/` 一致。两入口 `test_prod_deep.py`（三主题导出像素 + 用户字体）+ `test_cover_slots_export_local.py`（封面槽位 8 项像素断言）全绿。Cloudflare 首跑曾复现 v1.9.0 的代理黑洞事故签名（首个导出等 download 120s 超时），复跑即全绿，纯环境故障 |
| 本地归档 | 当前完整构建在 `app/dist/`；v1.10.0 快照在 `archive/dist-v1.10.0/`（9.6M，排除字体 3337 个）；v1.9.0 快照在 `archive/dist-v1.9.0/`（发版时漏交 git，2026-08-14 已补交 `0e9bb1e`）；v1.8.2 / v1.8.0 快照在 `archive/dist-v1.8.2/`、`archive/dist-v1.8.0/`；v1.7.3 / v1.7.2 / v1.7.0 / v1.5.1 / v1.5.0 / v1.4.1 / v1.4.0 归档继续保留，完整复原走对应 tag + `bash ci.sh` |
