...
| 最近完成迭代 | **v1.11.0「封面副标题微排版」2026-08-19 已双轨上线，2026-08-20 已完成公告闭环**。产品/视觉/发布提交为 `919802a` / `2052473` / `5c6ccf9`，精确 tag 为 `v1.11.0`。双入口与本地逐字节同构，两组 `test_prod_deep` 和两组公考 3×3 真导出矩阵均全绿；`archive/dist-v1.11.0/` 已归档。12:19（北京时间）已使用 `default` 租户「Claude聊天助手」回复刘彦君既有 1v1 发布会话发送公告，消息 `om_x100b675c4a2154a0c326e72ea070b91` 已回读确认。 |
...
- 授权边界：规划阶段不得 bump、tag、归档、部署或发公告；用户明确说「走上线闭环」「可以发布」或同义表达后，才执行发布。对本项目而言，该授权包含使用 `default` 租户「Claude聊天助手」向刘彦君既有 1v1 私聊发送专业公告；发送前仍须核验 profile/app/chat，发送后回读全文和真实消息 ID，不发群聊或其他联系人。若用户当次明确说只备稿或不发送，则以当次指令为准。
...
4. 只有用户另行明确授权，才 bump 版本、提交、tag、双轨部署、双生产入口回归、不覆盖归档并完成机器人私聊公告；本版在「我们走上线闭环」后执行。公告使用固定机器人和既有 1v1 会话，仍须在写操作前核验身份、写后回读，不扩散到群聊或其他联系人。
...
8. **发版公告必须专业、完整**（含工具网址 + 本版变化摘要 + 必要注意事项）。用户明确授权「走上线闭环」「可以发布」或同义表达时，公告属于闭环固定步骤：使用飞书旧企业 `default` 租户的「Claude聊天助手」发送到刘彦君既有 1v1 私聊，无需再索取“代发”二次口令；发送前核对 profile、app_id 与 chat，发送后回读全文并只记录真实 API 回执 ID。不得发群聊或其他联系人，用户自行转发；若用户当次明确要求只备稿或不发送，则不调用机器人
...
- 发版公告要写得专业、可直接发送；用户明确授权「走上线闭环」「可以发布」或同义表达时，固定机器人 1v1 公告属于闭环步骤，无需重复询问“代发”。发送前核对 `default` profile、「Claude聊天助手」与刘彦君既有私聊，发送后回读真实消息 ID；不发群聊或其他联系人，用户自行转发；当次明确只备稿或不发送时不发
- 终端统一 iTerm2
