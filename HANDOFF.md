...
> 最后更新：2026-08-11（v1.7.0「导入文稿与完整导出版」已完成生产候选实现、本地自动化与真实点击自查，尚待用户目检；**未部署、未打 tag、未发公告**。线上稳定版仍为 v1.5.1。旧版全文在 git 历史，`git log -- HANDOFF.md` / `git show <commit>:HANDOFF.md` 可考古）
...
| 状态 | **线上 v1.5.1 双轨稳定；本地分支 `codex/v1.7.0-import-markdown` 为 v1.7.0 生产候选**。候选已实现导入、同一可编辑新草稿、独立发布文案、17/18/19+ 边界、一次完整目录/ZIP 导出、自选页码、续写剩余页和中文正文两端对齐；尚未进入部署门禁 |
| 当前迭代 | **v1.7.0 实现与本地验证完成，等待用户目检截图和本地预览**。确认后才允许合并 `main`、推 Cloudflare + OSS/CDN、跑生产导出回归、打精确 `v1.7.0` tag、归档并发送公告。v1.6 字体本地化/减重、字重档位、结构化封面未混入；原生长文管线未接入；37+ 上传分组整体重均衡只留标记 |
...
| 测试基线 | v1.7.0 候选：Vitest **37 文件 / 292 测试**、`tsc -b`、ESLint、Vite build 全绿；新增解析、17/18/19+、范围归一、无丢页/重复页、单文件夹/单 ZIP、清单、目录中断/清单失败续写、超长 UTF-8 文件名、同名普通文件碰撞、发布文案兼容与两端对齐断言。真实浏览器点击完成 18 页与 19 页 fixture、手动编辑+自动保存+撤销、19+ 二次确认、范围输入+缩略图同步、无效页码失败态；本地生产构建还通过旧三主题/用户字体、1/2/5 页真实 ZIP，以及 19 页约 196 MB 真实 ZIP（01..19、2160×3600、manifest 无丢页/重复）；应用控制台 0 error / 0 warning。两个线上入口回归尚未运行，因为尚未部署 |
...
| v1.7.0 导入文稿与完整导出 | 本地生产候选，未上线 | **90%** | 用户目检本地预览/截图后，执行双轨部署、生产导出回归、精确 tag、归档与公告 |
...
5. **v1.7.0 导入文稿与完整导出版（本地候选，待目检上线）**：Markdown / 纯文本导入、覆盖式解析确认、同一可编辑新草稿、右栏独立发布文案、普通文稿 `---` 全局确认、17/18/19+ 上传兼容提示、一次完整目录/ZIP 导出、自选页码、导出清单、同会话续写剩余页和中文正文两端对齐。图片资源映射、原生长文、37+ 上传分组整体重均衡不在本次范围
...
- 真实交付：`test_v170_import_export_ui.py` 在全新 Chromium context 中下载约 196 MB 的 19 页兼容 ZIP；唯一顶层目录，PNG `01..19` 无丢失/重复、均为 2160×3600，manifest `sourcePages=1..19`，console/page error 均为 0。结果见 `/private/tmp/xhs-v170-import-export-ui/20260811T145004Z/result.json`
- 旧链回归：更新后的 `test_v150_prod.py` 与 `test_prod_deep.py` 已在本地 v1.7.0 生产构建跑通 1/2/5 页 ZIP、三旧主题、用户字体、2160×3600 像素与字形中线
- 候选构建：`assets/index-BKYNhwYN.js` SHA-256 `fbfafa1e…0dfc`；`assets/index-BVsea9B1.css` SHA-256 `39a1a1db…1c13`。正式部署若不再改代码，两入口与归档应精确匹配
- 版式：实际 DOM 画布为 1080×1800；正文计算样式 `justify / text-align-last:left / inter-character`，标题 `start / auto`；页面无横向溢出
- 控制台：本地生产预览应用日志 0 error / 0 warning
- 截图：`docs/screenshots/v1.7.0/01-import-18-review.png` 至 `06-export-19-selection.png`

### 明确未做 / 延后

- 没有部署、打 `v1.7.0` tag、归档或发送公告；必须先过用户视觉门禁
...
- 旧 v1.5.1 若在回滚后重新编辑导入草稿，旧代码可能丢掉可选 `publication` 元数据；Tiptap 图片正文仍可见。正式部署前应接受这一窄回滚权衡

### 新任务开场提示

```
继续小红书排版编辑器。线上稳定版仍为 v1.5.1；分支 `codex/v1.7.0-import-markdown` 已完成 v1.7.0 生产候选实现、本地自动化、真实 18/19 页点击验收和截图，但还没有部署、tag、归档或发送公告。下一步先等待用户目检本地预览；确认后按 §2 完成双轨上线闭环。开始前请先读：
...
v1.5.0 与 v1.5.1 的 tag 均已固定，不重做、不移动。用户视觉确认前不得部署 v1.7.0；确认后必须同时推 Cloudflare 与 OSS/CDN，并在两个生产入口实测导入、18/19 页状态、目录/ZIP 导出、旧主题和 2160×3600 像素输出，再打精确 tag 与归档。不要把 v1.6 或原生长文混进本版。
```
