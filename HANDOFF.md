# v1.14.0 发布进行中（2026-09-17 北京时间）

用户已实际审阅 rc.2 并明确授权上线闭环。发布内容是整个 Logo 优化：整篇四规则、逐页覆盖、统一设置区、三档封面位置；手动选页行为保持不变。开始正式版本构建、双轨部署、生产验证与使用说明同步。下方候选阶段的「无发布授权」已由本次授权取代；完成状态以之后的生产证据为准。

---

# 最新：v1.14.0-rc.2 本地候选（2026-09-17 北京时间，未发布）
