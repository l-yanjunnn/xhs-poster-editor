...
> 最后更新：2026-08-12（v1.7.2「确定性排版修复版」已完成用户确认、发布提交/tag、双轨部署、生产回归与不可覆盖归档；仅待飞书公告回执写回。旧版全文在 git 历史，`git log -- HANDOFF.md` / `git show <commit>:HANDOFF.md` 可考古）
...
| 当前迭代 | **v1.7.2 发布闭环已通过技术门禁**。自动测试、类型检查、ESLint、构建、最小排版样例、真实 19 页与两个生产入口回归全部通过；只剩飞书公告回执写回。原生长文、字体全集本地化/减重/字重档位/结构化封面、37+ 上传分组整体重均衡与跨刷新目录续写继续延期 |
| v1.7.2 发布闭环 | **2026-08-12 技术发布全部完成**：`v1.7.1` 初次生产门禁发现列表序号/正文中线 12px 回归，保留历史标签并升为 v1.7.2 修复；两入口同构建、1/2/5 页、公考主题、三旧主题、用户字体、撤销重做、标题/安全区、18/19 页导入与各 19 张真实 ZIP 全绿，H2 误差 0.5px、列表误差 0px；`archive/dist-v1.7.2/` 已生成 |
